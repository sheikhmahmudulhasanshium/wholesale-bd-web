...
class ApiClient {
  ...
  private handleGlobalError(error: AxiosError<ApiErrorResponse>) {
    if (typeof window === 'undefined') {
      console.error('[API Client Error - Server-side]:', error.response?.data?.message || error.message);
      return;
    }
  ...
  products = {
    create: (data: Partial<Product>): Promise<{ data: Product }> =>
      this.instance.post('/products', data),
    getAll: (
      params: ProductQuery,
      signal?: AbortSignal,
    ): Promise<{ data: PaginatedProductsResponse }> =>
    ...
    ): Promise<{ data: Product }> => this.instance.patch(`/products/${id}`, data),
    delete: (id: string): Promise<void> =>
      this.instance.delete(`/products/${id}`),
    getCategories: (): Promise<{ data: Category[] }> =>
      this.instance.get('/categories'),
  ...
  cart = {
    get: (): Promise<{ data: Cart }> => this.instance.get('/cart'),
    addItems: (items: { productId: string; quantity: number }[]): Promise<{ data: Cart }> =>
  ...
  orders = {
    createFromCart: (): Promise<{ data: Order }> => this.instance.post('/orders'),
    getAllForUser: (): Promise<{ data: Order[] }> => this.instance.get('/orders'),
    getByIdForUser: (orderId: string): Promise<{ data: Order }> =>
      this.instance.get(`/orders/${orderId}`),
  };
